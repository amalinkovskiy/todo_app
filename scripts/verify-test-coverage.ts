...
interface Result {
  featureFile: string;
  scenario: string;
  status: 'ok' | 'missing-metadata' | 'file-not-found' | 'test-not-found';
  message?: string;
}

function loadReportTests(): Map<string, Set<string>> {
  if (!fs.existsSync(REPORT_PATH)) {
    console.error(`Playwright report not found at ${REPORT_PATH}. Run tests first.`);
    process.exit(1);
  }
  const json: PlaywrightReport = JSON.parse(fs.readFileSync(REPORT_PATH, 'utf-8'));
  const mapping = new Map<string, Set<string>>(); // file -> Set(test titles)

  function walkSuites(suites: Suite[]): void {
...
function parseFeatureFile(filePath: string): Scenario[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('## Scenario:')) {
      ...
      if (m && m[1] && currentScenario) currentScenario.testFile = m[1].trim();
    } else if (line.toLowerCase().startsWith('test name:')) {
      const m = line.match(/test name:\s*`?([^`]+)`?/i);
      if (m && m[1] && currentScenario) currentScenario.testName = m[1].trim();
    }
  }
  if (currentScenario) scenarios.push(currentScenario);
  return scenarios;
    ...
    for (const sc of scenarios) {
      if (!sc.testFile || !sc.testName) {
        ...
        results.push({ 
          ...
          message: 'Test file or name missing' 
        });
        continue;
      }
      ...
      if (!testFileSet) {
        ...
        results.push({ 
          ...
          message: `Test file ${normalized} not present in report` 
        });
        continue;
      }
      if (!testFileSet.has(sc.testName)) {
        ...
        results.push({ 
          ...
          message: `Test name '${sc.testName}' not executed` 
        });
        continue;
      }
